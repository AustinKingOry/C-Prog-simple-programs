...
int main(){
	FILE *p;
	p = fopen("employees.txt","a+");

	//to check of the file already exists
	if (access("employees.txt",F_OK)!=-1){	
		printf("file exists\n");
	}
	else{
		printf("file does not exist.\n");
		fprintf(p,"IDNO\t NAME\t SALARY\t NgHIF\t NSSF\t NETPAY\t\n");
	}
...
int delRecord(FILE *recfile,int idDel){
	recfile = fopen("employees.txt","rw");
	char line[255];
	int y;
	for(y=0;y<idDel;y++){
		fgets(line,255,recfile);
